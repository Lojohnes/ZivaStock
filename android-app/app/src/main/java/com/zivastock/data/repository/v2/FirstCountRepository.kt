...
import com.zivastock.data.remote.dto.v2.FirstCountDto
...
class FirstCountRepository @Inject constructor(
    ...
    private val dao = database.firstCountDao()
    private val dateFormat = SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss", Locale.getDefault())
    private val queueTable = "v2_first_counts"

    fun getAll(): Flow<List<FirstCountEntity>> = dao.getAll()

    suspend fun getById(id: Long): FirstCountEntity? = dao.getById(id)

    suspend fun save(entity: FirstCountEntity): Long {
        ...
        auditLogRepository.log(
            ...
            entityId = id,
            status = "success",
            message = "First count saved locally",
            details = "sessionId=${entity.sessionId}, productId=${entity.productId}"
        )

        tryPush(entities = listOf(entity.copy(id = id)))
        return id
    }
    ...
    private suspend fun tryPush(entities: List<FirstCountEntity>): Result<Int> {
        ...
        return try {
            ...
            val response = api.createFirstCounts(dtos)
            ...
            if (response.isSuccessful && response.body() != null) {
                ...
                auditLogRepository.log(
                    ...
                    entityType = queueTable,
                    status = "success",
                    message = "Pushed ${entities.size} first count(s)",
    ...
    private suspend fun logFailure(entities: List<FirstCountEntity>, reason: String) {
        entities.forEach {
    ...
    suspend fun saveAll(entities: List<FirstCountEntity>) {
        dao.insertAll(entities)
    }

    suspend fun delete(entity: FirstCountEntity) {
        dao.delete(entity)
    }

    suspend fun fetchFromServer() {
        // First counts are only pushed from mobile
    }

    private fun FirstCountEntity.toDto(): FirstCountDto {
        return FirstCountDto(
